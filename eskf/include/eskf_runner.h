...
namespace eskf {
    ...
    class ESKFRunner {
    public:
        explicit ESKFRunner(ESKF &eskf) : _eskf(eskf) {
    ...
    void ESKFRunner<DELAYS>::update() {
        // imu更新了同时意味着imu_buffer非空
        if (_imu_updated) {

            // 只有当imu_buffer满了之后才运行eskf, 以保证永远在滞后的时刻进行融合
            if (_imu_buffer.is_full()) {
...
//            _eskf.set_dt(0.99f * _eskf._dt + 0.01f * _dt_imu);
    ...
    void ESKFRunner<DELAYS>::set_imu_data(const Vector3f &w, const Vector3f &a, const unsigned long &time_us) {
        ...
        if (_imu_buffer.is_empty()) {
            ...
            dt = 1e-6f * float(time_us - _imu_sample_last.time_us);
